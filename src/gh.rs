...
const QUERIES: &'static [&'static str] = &["https://api.github.com/search/repositories?q=language:rust&sort=stars&order=asc",
...
pub fn get_candidate_repos() -> Result<Vec<String>> {
    ...
    'next_query: for q in QUERIES {
        for page in 1..(QUERIES_PER + 1) {
            ...
            let response = String::from_utf8(buf).chain_err(|| "non-utf8 github response")?;
...
pub fn is_rust_app(name: &str) -> Result<bool> {
    ...
    let is_app = if let Ok(buf) = dl::download_no_retry(&url) {
        if let Ok(content) = String::from_utf8(buf) {
            ...
            if !content.contains("404: Not Found") && !content.is_empty() {
                true
            } else {
                false
            }
